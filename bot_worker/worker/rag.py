...
_SYNONYMS: dict[str, list[str]] = {
    ...
    "привыкани": ["зависимост", "привыкн"],
    "передозировк": ["передоз", "перебор"],
    "лекарств": ["таблетк", "медикамент", "препарат"],
    "врач": ["доктор", "медик"],
    "перерыв": ["пауза", "прекрат", "прервать"],
    "покраснени": ["флэш", "мурашк", "покалыван", "жжени"],
    "производител": ["фабрик", "завод", "качеств", "доверя"],
    "наук": ["микронутриентолог", "исследован", "изуча"],
    "акци": ["action", "экшн", "скидк", "промо", "спецпредложени"],
}

# ─── Program trigger keywords → program header prefix ───
...
# ─── Compact header (always included instead of full 5K header) ───
...
## ОПТИМАЛЬНЫЙ СЕТ
Оптимальный Сет — базовый набор (80% суточной нормы). Рекомендуй 1-2 раза за диалог, не больше.
...
# ─── Parsed knowledge base data ───

class _KBData:
    __slots__ = ("products", "product_keywords", "programs",
                 "registration", "intake", "business", "reviews", "prices",
                 "faq", "faq_keywords", "action_pack", "raw")
    ...
    def __init__(self):
        ...
        self.reviews: str = ""
        self.prices: str = ""
        self.faq: dict[str, str] = {}       # title -> text
        self.faq_keywords: dict[str, set[str]] = {}  # title -> keywords
        self.action_pack: str = ""
        self.raw: str = ""
...
def _parse_kb(text: str) -> _KBData:
    ...
    for chunk in chunks:
        ...
        if product_match:
            ...
            # Auto-extract keywords from ПОДХОДИТ ДЛЯ + КАТЕГОРИЯ + name
        ...
        if '## КОМПЛЕКСНЫЕ ПРОГРАММЫ ЗДОРОВЬЯ' in chunk:
            ...
            kb.prices = chunk
        elif '## АКЦИИ И СПЕЦПРЕДЛОЖЕНИЯ' in chunk:
            kb.action_pack = chunk
        elif '## ЧАСТО ЗАДАВАЕМЫЕ ВОПРОСЫ' in chunk:
            # Parse individual FAQ entries
            faq_entries = re.split(r'\n(?=### FAQ:)', chunk)
            for entry in faq_entries:
                faq_m = re.match(r'### FAQ:\s*(.+?)(?:\n|$)', entry)
                if faq_m:
                    title = faq_m.group(1).strip()
                    kb.faq[title] = entry.strip()
                    # Extract keywords from title
                    kws = {w.lower() for w in re.sub(r'[^\w\s]', ' ', title).split() if len(w) > 3}
                    kb.faq_keywords[title] = kws

    return kb


# ─── Word extraction & expansion ───
...
def select_relevant_kb(user_message: str, chat_history: list[dict] | None = None) -> str:
    """
    Select relevant KB sections based on user message and recent chat history.
    Returns compact KB string (~2000-3000 tokens instead of ~15000).
    """
    kb = _get_kb()

    if not kb.products:
        return kb.raw  # fallback — return full KB

    # Build query from user message + last 4 history messages
    ...
    if kb.faq:
        for title, kws in kb.faq_keywords.items():
            faq_score = 0
            for ew in expanded:
                for kw in kws:
                    if ew == kw:
                        faq_score += 3
                    elif len(ew) >= 4 and len(kw) >= 4:
                        if ew in kw or kw in ew:
                            faq_score += 2
                        elif ew[:4] == kw[:4]:
                            faq_score += 1
            if faq_score >= 2:
                faq_matches.append(title)

    # Action Pack intent
    action_words = {"акци", "action", "экшн", "промо", "спецпредложени", "экшнпак"}
    wants_action = any(
        aw in w or w in aw
        for w in expanded
    ...
    # Compact catalog overview (always — so bot knows full lineup)
    parts.append(_catalog_overview(kb, selected))

    # Detailed product cards for selected products
    ...
    # Registration options (always — needed for closing sales)
    if kb.registration:
        parts.append(kb.registration)

    # Intake recommendations (always — for consultations)
    if kb.intake:
    ...
    # FAQ entries (when user asks questions matching FAQ)
    if faq_matches:
        faq_parts = [kb.faq[t] for t in faq_matches[:3] if t in kb.faq]
        if faq_parts:
            parts.append("## ОТВЕТЫ НА ЧАСТЫЕ ВОПРОСЫ\n\n" + "\n\n".join(faq_parts))

    # Action Pack (when user asks about promotions)
    if wants_action and kb.action_pack:
        parts.append(kb.action_pack)
