...
import { Shield, Users, Bot, BarChart3, Search, Trash2, Eye, ToggleLeft, ToggleRight, ChevronLeft, ChevronRight, MessageSquare, X, AlertTriangle, ArrowLeft, ArrowDown, Handshake, Settings } from 'lucide-react'
import api from '../api'
...
function StatsTab() {
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <StatCard label="Реф. привязки" value={stats.total_referrals} sub={`${stats.users_with_referrals} пригласивших`} />
    </div>
  )
}
...
function ReferralsTab() {
  ...
  const load = useCallback(() => {
    ...
    api.get('/api/admin/referrals', { params: { search, limit, offset: page * limit } })
      .then(r => { setReferrals(r.data.referrals); setTotal(r.data.total) })
      .finally(() => setLoading(false))
  }, [search, page])
      ...
      {loading ? (
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="text-white/30 text-xs border-b border-white/[0.06]">
                <th className="text-left py-3 px-3 font-medium">Реферал</th>
                <th className="text-left py-3 px-3 font-medium">Пригласил</th>
                <th className="text-left py-3 px-3 font-medium">Дата регистрации</th>
              </tr>
            </thead>
            <tbody>
              {referrals.map(r => (
                <tr key={r.id} className="border-b border-white/[0.04] hover:bg-white/[0.02] transition-colors">
                  <td className="py-3 px-3">
                    <div className="text-white/70 text-xs">{r.name}</div>
                    <div className="text-white/30 text-xs">{r.email}</div>
                  </td>
                  <td className="py-3 px-3">
                    <div className="text-white/70 text-xs">{r.referrer_name || '—'}</div>
                    <div className="text-white/30 text-xs">{r.referrer_email || ''}</div>
                  </td>
                  <td className="py-3 px-3 text-white/30 text-xs">{r.created_at ? new Date(r.created_at).toLocaleDateString('ru') : '—'}</td>
                </tr>
              ))}
            </tbody>
          </table>
