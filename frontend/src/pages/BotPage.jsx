...
export default function BotPage() {
  ...
  if (!bot) {
    return (
      <div>
        <h1 className="text-2xl font-bold mb-8">Мой бот</h1>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <h1 className="text-2xl font-bold">Мой бот</h1>
      ...
      {/* Bot link — prominent */}
      {bot.bot_username && (
        <div className="glass-card p-5 mb-6 border-l-4 border-l-accent-500">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <a href={`https://t.me/${bot.bot_username}`} target="_blank" rel="noopener noreferrer"
              className="text-lg font-semibold text-accent-400 hover:text-accent-300 hover:underline break-all">
        ...
        <div className="flex items-center justify-between py-3 px-1">
          ...
          <button type="button" onClick={() => setAllowPartners(!allowPartners)}
            className={`relative w-11 h-6 rounded-full transition-colors ${
              allowPartners ? 'bg-accent-500' : 'bg-dark-600'
            }`}>
                        ...
                        <td className="py-2.5">
                          {p.seller_link ? (
                            <a href={p.seller_link} target="_blank" rel="noopener noreferrer"
                              className="text-accent-400 hover:underline truncate block max-w-[200px]">
                          ...
                          {addingCredits === p.id ? (
                            <div className="flex items-center gap-2 justify-end">
                              <input type="number" min="1" max="1000" value={creditsAmount}
                                ...
                                className="w-16 bg-dark-600 border border-white/10 rounded px-2 py-1 text-sm text-white text-center" />
                            ...
                            <button onClick={() => { setAddingCredits(p.id); setCreditsAmount(5) }}
                              className="flex items-center gap-1 text-accent-400 hover:text-accent-300 text-xs ml-auto">
