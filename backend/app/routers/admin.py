...
from app.models import User, Bot, Contact, Message, Broadcast, PasswordResetToken
...
async def admin_stats(
    ...
    users_with_referrals = (await db.execute(
...
async def admin_list_referrals(
    ...
    """List all referral bindings (users with referred_by_id set)."""
    query = select(User).where(User.referred_by_id.isnot(None))
    count_query = select(func.count(User.id)).where(User.referred_by_id.isnot(None))
    ...
    referrer_ids = list({u.referred_by_id for u in users if u.referred_by_id})
    referrers = {}
    if referrer_ids:
        ...
        "referrals": [
            {
                ...
                "referrer_name": referrers.get(u.referred_by_id, None) and referrers[u.referred_by_id].name,
                "referrer_email": referrers.get(u.referred_by_id, None) and referrers[u.referred_by_id].email,
                "created_at": u.created_at.isoformat() if u.created_at else None,
            }
            for u in users
        ],
