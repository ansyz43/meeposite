...
[
  {{
    ...
    "hashtags": "#тег1 #тег2",
    "time": "09:00",
    "script": "Текст для озвучки (ТОЛЬКО для рилс и сторис, для постов и каруселей — пустая строка)"
  }},
  ...
]

## ПРАВИЛА СКРИПТА ДЛЯ ОЗВУЧКИ (поле "script"):
- Генерируй скрипт ТОЛЬКО для типов "рилс" и "сторис". Для "пост" и "карусель" — пустая строка ""
- Скрипт — это ГОТОВЫЙ текст, который человек читает на камеру / озвучивает голосом
- Пиши разговорным живым языком, как будто говоришь с подругой
- БЕЗ эмодзи, БЕЗ хештегов, БЕЗ markdown-форматирования
- Начинай с сильного крючка (hook) — первая фраза должна останавливать
- Рилс: 30-60 секунд (150-350 слов). Сторис: 10-20 секунд (40-100 слов)
- Используй короткие предложения. Ставь паузы через точки или многоточия
- Заканчивай призывом: вопрос аудитории / CTA / интрига
- Тон: {profile.tone}, но адаптированный под устную речь
...
Сгенерируй ровно {period_days} постов."""
        ...
        for item_data in posts_list:
            ...
            item = ContentPlanItem(
                ...
                script=item_data.get("script") if post_type in ("рилс", "сторис") else None,
            )
            db.add(item)
